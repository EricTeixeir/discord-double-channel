...
function resolveAutoOffMs(): number {
...
function setRelayActive(active: boolean): void {
...
export function isRelayActive(): boolean { return relayActive; }

export async function setupRelay(clientA: Client<true>, clientB: Client<true>): Promise<void> {
  // Cada client busca o guild pela própria sessão: o adapter de voz é por client.
  const [guildA, guildB] = await Promise.all([
    clientA.guilds.fetch(env.GUILD_ID).catch(() => null),
    clientB.guilds.fetch(env.GUILD_ID).catch(() => null),
  ]);
  if (!guildA) throw new Error(`bot A (${clientA.user.tag}) não está no servidor ${env.GUILD_ID} — convide-o pela URL do OAuth2`);
  if (!guildB) throw new Error(`bot B (${clientB.user.tag}) não está no servidor ${env.GUILD_ID} — convide-o pela URL do OAuth2`);
...
function attachAutoReconnect(connection: VoiceConnection, label: string): void {
...
function wireDirection(botIds: Set<string>, sourceConn: VoiceConnection, targetConn: VoiceConnection): void {
  ...
  relayEvents.on('change', (active: boolean) => {
    if (!active) {
      stopOutput();
      return;
    }
